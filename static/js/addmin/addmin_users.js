document.addEventListener('DOMContentLoaded', function() {
    ...
    class UserManager {
        ...
        renderVolunteerItem(containerId, volunteer) {
            ...
            // 空 ID 防呆：志愿对象存在但 id 缺失（例如后端还没写入）时，灰按钮不可点，避免拼出非法 URL
            const hasValidId = volunteer.id !== undefined && volunteer.id !== null && volunteer.id !== '' &&
                               String(volunteer.id) !== 'null' && String(volunteer.id) !== 'undefined';
            ...
            // 根据志愿状态生成操作按钮
            let actionsHtml = '';
            const disabledAttr = hasValidId ? '' : ' disabled style="opacity:.6;cursor:not-allowed"';
            const invalidIdTooltip = hasValidId ? '' : ' title="请先保存志愿信息，再进行排队操作"';
            if (volunteer.status === 'WAITING') {
                if (isInQueue) {
                    actionsHtml = `
                        <button class="btn btn-sm btn-danger volunteer-action-btn"${disabledAttr}${invalidIdTooltip}
                                data-volunteer-id="${volunteer.id}"
                                data-action="cancel_queue">取消排队</button>
                        <button class="btn btn-sm btn-warning volunteer-action-btn"${disabledAttr}${invalidIdTooltip}
                                data-volunteer-id="${volunteer.id}"
                                data-action="requeue">重新排队</button>
                    `;
                } else {
                    actionsHtml = `
                        <button class="btn btn-sm btn-success volunteer-action-btn"${disabledAttr}${invalidIdTooltip}
                                data-volunteer-id="${volunteer.id}"
            ...
            container.innerHTML = `
                <div class="volunteer-info-left">
                    ...
                    ${!hasValidId ? `<span class="volunteer-status empty" style="margin-left:8px">⚠ 请先保存志愿</span>` : ''}
                </div>
        ...
        async handleVolunteerAction(volunteerId, action) {
            ...
            // 防呆 1：空 / null / undefined / 'null' / 'undefined' / NaN 一律不发请求
            const invalidId =
            ...
            if (invalidId) {
                alert(`无法${actionLabel}：该志愿尚未保存，请先在「个人中心」填写并保存志愿信息，再执行此操作。`);
                return;
            }
