...
ROLE_HOME_URL_NAMES = {
    ...
    Role.INTERVIEWER: 'interviewer_panel',
    Role.SUBADMIN: 'subadmin_console',
    Role.ADMIN: 'admin_console',
    Role.SUPER_ADMIN: 'admin_console',
}
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return JsonResponse({
                ...
                'message': '登录成功',
                'role': role.value,
                'redirect_url': reverse(home_url_name),
            })
        else:
...
def new_feature_view(request):
    """新功能占位页：登录后所有角色可访问（sidebar 各角色均有入口，避免出现 404）"""
