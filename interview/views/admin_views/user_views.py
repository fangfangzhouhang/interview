...
def admin_users_view(request):
    """管理员用户管理视图"""
    current_role = RoleManager.get_user_role(request.user)
    is_superuser = current_role == Role.SUPER_ADMIN
    is_admin = current_role == Role.ADMIN
...
def api_admin_users(request):
    ...
    if request.method == 'GET':
        ...
        all_data = []
        for user in users:
            user_role = RoleManager.get_user_role(user).value
...
def api_admin_user_detail(request, user_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            current_user_role = RoleManager.get_user_role(request.user).value
            ...
            if role is not None:
                ...
                target_user_role = RoleManager.get_user_role(user).value
                ...
                if target_user_role == ROLE_SUPER_ADMIN and request.user.id != user_id:
...
def api_admin_user_reset_password(request, user_id):
    ...
    current_user_role = RoleManager.get_user_role(request.user).value
    if user_role == ROLE_SUPER_ADMIN and current_user_role != ROLE_SUPER_ADMIN:
