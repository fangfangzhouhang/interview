...
@subadmin_required()
def subadmin_scores_view(request):
    """部门管理员评价信息管理视图"""
    return render(request, 'subaddmin/subaddmin_scores.html')


@csrf_exempt
@subadmin_required(return_json=True)
@department_permission_required(return_json=True)
def api_subadmin_scores(request, department=None):
    """获取评价信息列表 - 自动过滤只显示本部门的评价"""
    if request.method == 'GET':
        ...
        # 根据部门过滤 - 通过 interview_group 的 departments 字段
        if department:
            if department != 'ALL':
        ...
        if status:
            scores = scores.filter(interview_group__status=status)
...
@subadmin_required(return_json=True)
@department_permission_required(return_json=True)
def api_subadmin_scores_options(request, department=None):
    """获取评价管理所需的选项数据（状态）"""
    if request.method != 'GET':
        return JsonResponse({'success': False, 'message': '请求方法错误'})
...
@subadmin_required(return_json=True)
@department_permission_required(return_json=True)
def api_subadmin_group_scores(request, group_id, department=None):
