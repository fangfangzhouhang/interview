...
from ...permissions.roles import RoleManager
...
def _get_user_role(user):
    """获取用户角色"""
    return RoleManager.get_user_role(user).value
...
def api_subadmin_user_detail(request, user_id, department=None):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # 获取当前用户的角色
            current_user_role = _get_user_role(request.user)
            ...
            # 2. 检查目标用户当前角色
            target_user_role = _get_user_role(target_user)
